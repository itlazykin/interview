...
[Алгоритм бинарного поиска (двоичное дерево поиска)](#алгоритм-бинарного-поиска)
...
+ Хеш-функция может генерировать большое количество различных хеш-кодов для различных объектов. 
+ Однако, поскольку количество бакетов в хеш-таблице ограничено (например, 16, 32 или 64 бакета), вероятность того, что
два различных хеш-кода приведут к одному и тому же индексу бакета, существует. 
+ Это происходит из-за операции деления хеш-кода на количество бакетов (или, в случае использования степени двойки,
операции побитового И с (capacity - 1)).
...
#### Как работает красно-черное дерево при коллизиях в HashMap:

- Инициализация: Если два или более элемента в одном бакете приводят к коллизии (то есть их хеш-коды совпадают), они
хранятся в связном списке. Но когда количество элементов в бакете превышает порог (обычно 8), элементы в этом бакете
  преобразуются в красно-черное дерево. В красно-черном дереве элементы (узлы) красатся в красный или черный цвет в
  процессе вставки и удаления, чтобы сохранить баланс дерева и соответствовать всем его свойствам. Когда элемент
  добавляется в дерево, он всегда первоначально вставляется как красный узел, и затем происходит серия перекрасок и
  поворотов для восстановления свойств красно-черного дерева. Красный узел не может иметь красных детей, Если при
  вставке или после поворота два красных узла оказываются рядом (родитель и его ребенок), то необходимо выполнить
  перекраску и повороты, чтобы восстановить баланс. При вставке нового элемента он всегда будет красным, но корень
  дерева всегда должен быть черным. Если после вставки корень становится красным, его нужно перекрасить в черный. Черные узлы должны быть сбалансированы по всей высоте дерева, чтобы все пути от корня до листа содержали одинаковое количество черных узлов

- Вставка: При вставке нового элемента в красно-черное дерево, элемент сначала добавляется как красный узел.
После этого дерево может быть сбалансировано с помощью поворотов (влево или вправо) и перекраски узлов, чтобы
удовлетворять всем правилам красно-черного дерева.

- Поиск:
Поскольку красно-черное дерево является сбалансированным, операции поиска выполняются за время O(logN), где N — количество элементов в дереве.
- Для поиска элемента мы начинаем с корня и следуем по дереву, сравнивая ключи и перемещаясь влево или вправо в
зависимости от результата сравнения.

- Удаление: При удалении элемента из красно-черного дерева также выполняются корректировки, чтобы сохранить баланс дерева и соблюсти
все правила красно-черного дерева.

- Балансировка дерева:
- Когда вставляется новый узел, дерево может потребовать балансировки. Для этого используются повороты:
  - Левый поворот: Смена местами узлов, чтобы уменьшить высоту правого поддерева. 
  - Правый поворот: Смена местами узлов, чтобы уменьшить высоту левого поддерева. 
  - Эти операции гарантируют, что дерево остается сбалансированным, и каждая операция будет выполняться за логарифмическое
время.
...
+ O(1) – константная сложность (т.е. константное время); Время выполнения не зависит от размера входных данных. Алгоритм выполняется за постоянное время. Пример: доступ к элементу массива по индексу.
+ О(n) – линейная сложность; Время выполнения пропорционально количеству элементов в данных. Пример: поиск максимального элемента в массиве.
+ О(log(n)) – логарифмическая сложность; Алгоритм выполняет работу, которая уменьшается на каждую итерацию в зависимости от размера входных данных. Пример: бинарный поиск в отсортированном массиве.
+ O(n*log(n)) – Линейно-логарифмическая сложность; Обычно встречается в эффективных алгоритмах сортировки. Пример: алгоритм сортировки слиянием или быстрая сортировка (QuickSort)
+ O(n^2) — Квадратичная сложность. Время выполнения пропорционально квадрату размера входных данных. Часто встречается в алгоритмах с двумя вложенными циклами. Пример: сортировка пузырьком, сортировка выбором.
+ O(2n) – экспоненциальная сложность; Время выполнения растет экспоненциально с увеличением размера данных. Это очень неэффективный алгоритм. Пример: решение задачи о рюкзаке с полным перебором.
+ O(n!) – факториальная сложность. Время выполнения растет с факториальной скоростью. Обычно встречается в задачах перебора всех возможных вариантов. Пример: задача о коммивояжере.
