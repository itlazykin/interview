...
В лямбда-выражениях можно использовать параметры самой лямбды, локальные переменные метода (но только те, что final или effectively final - их нельзя изменять после инициализации. Потому что лямбда не копирует переменную, а захватывает её значение. Чтобы избежать непредсказуемого поведения, Java требует неизменяемости), а также поля объекта и статические переменные. Ограничение на локальные переменные связано с тем, что лямбда захватывает их значение, а не саму переменную, чтобы избежать ошибок при изменении.
