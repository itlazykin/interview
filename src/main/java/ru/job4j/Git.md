...
[4. Git init - как создать репозиторий](#4-git-init)

[5. Git clone](#5-git-clone)

[6. Git log - посмотреть историю коммитов](#6-git-log)
...
[19. Git revert - отменить коммит, отправленный в репу](#19-git-revert)

[20. Интерактивный rebase -i](#20-интерактивный-rebase--i)

[21. rebase vs merge](#21-rebase-vs-merge)

[22. Как переключиться на другую ветку](#22-как-переключиться-на-другую-ветку)

[23. Опишите процесс решения конфликтов при слиянии веток в Git.](#23-опишите-процесс-решения-конфликтов-при-слиянии-веток-в-git)

[24. git force push](#24-git-force-push)

[25. Какой командой можно просмотреть список всех веток, включая удаленные, и последние коммиты в них?](#)
...
Она сохраняет все незакоммиченные изменения (как в рабочем каталоге, так и в индексе) в специальное хранилище, после чего можно безопасно переключаться на другие ветки или выполнять другие операции. Чтобы вернуть сохранённые изменения, используется команда git stash apply или git stash pop, которая также удаляет изменения из хранилища

git stash apply накатывает изменения из stash, но оставляет его в списке. git stash pop делает то же самое, но ещё и удаляет stash — поэтому он используется, когда изменения больше не нужны в отложенном виде.
...
- Сдвигает HEAD на указанный коммит. 
- Index (staging area) и working directory остаются без изменений. 
- Все изменения после <commit> будут помечены как подготовленные к коммиту (в staging).

Полезно, если ты хочешь объединить несколько коммитов в один (commit squash вручную).

`git reset --mixed <commit> (по умолчанию)`
- Сдвигает HEAD. 
- Очищает staging area (файлы становятся «изменёнными», но не staged). 
- Working directory не трогает.

Это стандартный откат «верни изменения в рабочее состояние, как будто я их ещё не добавлял в индекс».

`git reset --hard <commit>`
- Сдвигает HEAD. 
- Полностью очищает staging area. 
- Удаляет изменения в рабочей директории. 
- Всё становится ровно как в <commit>.

Используется, если нужно «стереть» все локальные изменения.
...
Основное отличие между git rebase и git merge заключается в способе интеграции изменений из одной ветки в другую и во влиянии на историю коммитов.
В идеале: локально использовать rebase, а в main — merge.

[К оглавлению](#Git)

# 22. Как переключиться на другую ветку

Чтобы переключиться на другую ветку в Git, используется команда git checkout <имя_ветки>. Чтобы создать новую ветку и сразу на неё переключиться, используется команда git checkout -b <имя_ветки>. Если вы хотите просто создать новую ветку без переключения, используйте команду git branch <имя_ветки>

[К оглавлению](#Git)

# 23. Опишите процесс решения конфликтов при слиянии веток в Git.

Делаем мерж из дев ветки в свою feature 

[К оглавлению](#Git)

# 24. git force push

git push --force перезаписывает историю ветки на сервере так, чтобы она совпадала с локальной. Это может затереть чужие коммиты, поэтому безопаснее использовать git push --force-with-lease. Обычно применяют после rebase или amend в feature-ветках.
