...
Основные методы класса String в Java позволяют выполнять такие операции, как получение длины (length()), преобразование в массив символов (toCharArray()), сравнение строк (equals(), compareTo()), поиск символов и подстрок (indexOf(), contains()), извлечение подстрок (substring()), изменение регистра (toLowerCase(), toUpperCase()), удаление пробелов (trim()), объединение (concat()) и разделение (split()) строк. 

Получение информации о строке 
- length(): возвращает количество символов в строке.
- isEmpty(): возвращает true, если строка пустая, и false в противном случае.
- toCharArray(): возвращает строку в виде массива символов char[].
- getBytes(): преобразует строку в массив байт.

Поиск символов и подстрок
- indexOf(String str): возвращает индекс первого вхождения подстроки str в текущей строке. 
- lastIndexOf(String str): возвращает индекс последнего вхождения подстроки str. 
- contains(CharSequence s): возвращает true, если строка содержит заданную последовательность символов, и false в противном случае. 

Извлечение и манипулирование частями строки
- substring(int beginIndex): возвращает новую строку, начинающуюся с beginIndex. 
- substring(int beginIndex, int endIndex): возвращает новую строку, содержащую символы от beginIndex до endIndex (не включая endIndex). 
- split(String regex): разделяет строку на массив подстрок по заданному регулярному выражению. 

Сравнение строк 
- equals(Object anObject): сравнивает две строки с учетом регистра.
- equalsIgnoreCase(String anotherString): сравнивает две строки без учета регистра.
- compareTo(String anotherString): сравнивает две строки лексикографически.

Изменение строки 
- concat(String str): объединяет текущую строку с другой строкой.
...
Преобразование других типов в строку 
...
Основные методы класса StringBuffer в Java используются для работы с изменяемыми строками. Они включают: 
- length(): — возвращает текущую длину строки.
- capacity(): — возвращает текущую выделенную ёмкость (количество символов, которое может вместить объект без перераспределения памяти).
- append(): — добавляет различные типы данных в конец строки.
- insert(): — вставляет строку или символ в указанную позицию.
- delete() и deleteCharAt(): — удаляют символы или их последовательность из строки.
- replace(): — заменяет часть строки на другую подстроку.
- reverse(): — переворачивает строку.
- setLength(int length): — устанавливает новую длину строки.
- charAt(int index) и setCharAt(int index, char ch): — позволяют получить или установить символ по указанному индексу.
- substring(): — возвращает часть содержимого объекта как новую строку.
  
+ `StringBuilder` - изменяемый, потокоНЕбезопасный

Методы для добавления и вставки
- append(String str): Добавляет строку в конец объекта StringBuilder. 
- append(boolean b) / append(int i): Перегруженные методы append(), которые принимают различные типы данных, преобразуют их в строку и добавляют в конец. 
- insert(int offset, String str): Вставляет строку str в указанную позицию offset. 

Методы для удаления 
- delete(int start, int end): Удаляет символы в диапазоне от start до end.
- deleteCharAt(int index): Удаляет символ по указанному индексу index.

Методы для изменения и замены
- replace(int start, int end, String str): Заменяет часть строки в диапазоне start..end на переданную строку str. 
- reverse(): Переворачивает последовательность символов объекта StringBuilder. 

Другие полезные методы
- toString(): Возвращает новое строковое значение из объекта StringBuilder. 
- setLength(int newLength): Изменяет длину строки, обрезая или добавляя пробельные символы, если newLength меньше или больше текущей длины. 
- setCharAt(int index, char ch): Изменяет символ в указанной позиции index на символ ch. 
...
+ Bootstrap ClassLoader: Загружает базовые классы JDK `(java.lang.*, java.util.*)`. Написан на С/С++, не имеет родителя.
+ Extension ClassLoader: Загружает классы расширений JDK 
+ System ClassLoader: Загружает классы из classpath. 
+ Custom ClassLoader: Пользовательские загрузчики классов. Можно написать свой
