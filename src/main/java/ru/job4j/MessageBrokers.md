...
[21. Что такое Kafka lag](#21-что-такое-kafka-lag)
...
# 21. Что такое Kafka lag

Kafka lag (или lag в Kafka) — это разница между позицией (offset) сообщений, которые были записаны в топик Kafka, и позицией, на которой находится потребитель (consumer) этого топика.

Проще говоря, lag в Kafka — это количество сообщений, которые ещё не были обработаны потребителем, несмотря на то, что они уже записаны в топик Kafka. Это важный показатель, который позволяет оценить, насколько быстро потребитель обрабатывает данные, и позволяет отслеживать производительность системы в реальном времени.

#### Как работает lag в Kafka?
- В Kafka каждое сообщение в топике имеет уникальный offset (позицию), который представляет собой порядковый номер этого сообщения в партиции топика. 
- Когда потребитель считывает сообщения, он прибавляет к своему текущему offset (позиции), показывая, до какого сообщения он дошел. 
- Lag определяется как разница между текущим смещением последнего записанного сообщения в партицию и смещением последнего прочитанного сообщения потребителем.

Пример: Допустим, у вас есть топик с одной партицией, и в нем есть 1000 сообщений. Если потребитель прочитал до 800-го сообщения, то его смещение (offset) будет 800. Если в этот момент в топик записано еще 100 сообщений (от 801 до 900), то lag будет равен 100. То есть, потребитель отстал на 100 сообщений.
