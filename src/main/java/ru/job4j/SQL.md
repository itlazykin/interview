...
+ В JPA и Hibernate это реализуется с помощью аннотации @Version. В сущности добавляется специальное поле — счётчик версии или метка времени. При каждом изменении записи JPA генерирует запрос вида:
...
Если никто другой не обновил строку, версия совпадёт, обновление выполнится, и версия увеличится. Если же параллельная транзакция уже изменила данные и версия не совпадает, то Hibernate выбросит OptimisticLockException (или ObjectOptimisticLockingFailureException в Spring).

Этот механизм хорош тем, что не использует «тяжёлые» блокировки в базе и отлично подходит для систем, где конфликты редки, но важно их отлавливать. Обычно в сервисном слое либо возвращают ошибку клиенту (например, HTTP 409 Conflict), либо реализуют стратегию повторных попыток (retry) с перечитыванием свежих данных.

### Пессимистичная блокировка:

Блокирует данные, чтобы другие транзакции не могли их изменить. На уровне SQL это обычно транслируется в `SELECT ... FOR UPDATE`. Такая блокировка гарантирует, что пока одна транзакция работает с записью, другая транзакция не сможет её изменить или даже, в зависимости от режима, не сможет её прочитать.
