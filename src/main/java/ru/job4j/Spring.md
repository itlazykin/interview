...
Spring Framework — это мощный и гибкий фреймворк для разработки Java-приложений, который предоставляет множество
инструментов и модулей для управления зависимостями, безопасности, транзакциями, веб-разработки и тестирования. Он
использует принцип инверсии управления (IoC - Inversion of Control), Dependency Injection (внедрение зависимостей), а также включает Spring
MVC для веб-приложений, Spring Security для безопасности, Spring AOP для аспектно-ориентированного программирования,
Spring Data для работы с различными источниками данных.
...
Контейнер IoC в Spring по своей сути напоминает усовершенствованную Map. Ключи: Имена бинов (строковые идентификаторы или типы классов).
...
+ Singleton
  ...
  + Один бин на весь Spring-контейнер. 
  + Когда контейнер поднимается, создаётся один экземпляр класса. 
  + Все, кто внедряют этот бин через @Autowired, получают одну и ту же ссылку.
...
+ Prototype
  ...
  + НЕ ХРАНЯТСЯ в контексте Spring-а. Новый бин при каждом запросе к контейнеру. Каждый @Autowired или вызов context.getBean() создаёт новый объект.
  + применяется, когда объект хранит состояние и не должен шариться между потоками
...
Spring создаёт прокси-объект, который переопределяет методы.
private методы не переопределяются, т.к. они недоступны в наследниках.
Поэтому @Transactional не работает на private методах.
...
Аннотация @Scheduled в Spring Framework используется для выполнения методов по расписанию, что делает её полезной для планирования задач, таких как периодические обновления данных, очистка кэша, отправка уведомлений и другие задачи, которые должны выполняться с определенной периодичностью.

Она является частью Spring Task Scheduler, который предоставляет простой способ для асинхронного и периодического выполнения методов.

#### Как работает @Scheduled:
Аннотация @Scheduled позволяет методам выполняться через заданные интервалы времени или в определенные моменты. В методах, помеченных этой аннотацией, можно указать различные параметры для задания расписания.

Чтобы использовать @Scheduled, нужно включить поддержку планировщика задач в вашем Spring-приложении с помощью аннотации @EnableScheduling.

Аннотация @Scheduled позволяет задавать параметры для выполнения методов, такие как:
- fixedRate: Метод будет вызываться через определённые интервалы времени, начиная с момента начала последнего выполнения. 
- fixedDelay: Метод будет вызываться через определённый интервал после окончания последнего выполнения. 
- cron: Позволяет задать более сложное расписание с использованием cron-выражений.

#### Особенности использования
- Задачи, помеченные @Scheduled, выполняются в потоке планировщика задач и могут работать асинхронно. Важно учитывать, что если задача выполняется долго, это может повлиять на другие задачи, если планировщик ограничен в ресурсах
- Планировщик будет работать до тех пор, пока приложение активно. Если нужно остановить или изменить расписание, это можно сделать через конфигурацию Spring
- В случае более сложных задач планирования вы можете настроить другие параметры, такие как количество потоков или время выполнения через конфигурацию Spring
...
Аннотация @Cacheable в Spring Framework используется для кеширования результатов методов. Она позволяет сохранять результаты выполнения метода в кэше, чтобы при последующих вызовах с теми же параметрами результат мог быть извлечен из кэша, что значительно ускоряет выполнение, уменьшая нагрузку на систему.

- Spring поддерживает различные решения для кеширования, такие как ConcurrentMap, EhCache, Redis, Caffeine и другие, и позволяет легко интегрировать их в ваше приложение через аннотацию @Cacheable.
- Когда метод помечен аннотацией @Cacheable, Spring сохраняет результат выполнения этого метода в кэш. Если метод будет вызван с теми же аргументами, результат будет возвращён напрямую из кэша, минуя фактическое выполнение метода

#### Как работает кеширование?
- Когда метод с аннотацией @Cacheable вызывается в первый раз, Spring выполняет метод и сохраняет результат в кэш, используя параметры метода в качестве ключа. 
- Если метод вызывается с теми же аргументами, Spring извлекает результат из кэша вместо того, чтобы снова выполнять метод. Это может значительно ускорить выполнение, особенно если метод выполняет долгие операции (например, обращение к базе данных или внешний API). 
- Кэш управляется автоматически, Spring берет на себя всю работу по записи и извлечению данных из кэша.
