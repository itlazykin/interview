...
В Stream API в Java операции делятся на два типа: stateful (состоящие) и stateless (бессостоящие). Разница между ними заключается в том, как они обрабатывают элементы потока и могут ли они сохранять состояние между элементами потока.
...
Пример: map(), filter(), forEach(), distinct(), limit()
...
Пример: sorted(), reduce(), collect(), skip(), peek() 
