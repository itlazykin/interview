...
Промежуточные операции делятся на два типа: stateful (состоящие) и stateless (бессостоящие). Разница между ними заключается в том, как они обрабатывают элементы потока и могут ли они сохранять состояние между элементами потока.

- `Stateless операции` - это операции, которые не зависят от состояния элементов потока и не хранят промежуточные данные, а просто выполняют вычисления для каждого элемента независимо от остальных. Каждая операция применяется к отдельному элементу потока и не сохраняет информацию о предыдущих элементах.

Stateless операции работают быстро, так как они не требуют хранения состояния между элементами потока. Это означает, что операции выполняются независимо от других и могут быть легко распараллелены.

- `Stateful операции` - это операции, которые зависят от состояния предыдущих элементов потока и могут сохранять промежуточное состояние. Эти операции требуют дополнительной памяти или вычислений для обработки всех элементов потока. В случае с состоянием, такие операции часто требуют дополнительных ресурсов для обеспечения правильного порядка выполнения. 

Stateful операции более ресурсоемки, так как они требуют хранения состояния или сортировки элементов, что может замедлить работу потока и сделать его неподходящим для параллельного исполнения

Stateless — можно в параллельных стримах без проблем.

Stateful — могут вызвать накладные расходы (буферизация) или неожиданное поведение в параллельных стримах.

| Операция  | Тип       | Описание                                      |
|-----------|-----------|-----------------------------------------------|
| filter    | stateless | Обрабатывает каждый элемент независимо        |
| map       | stateless | Преобразует элемент → элемент                 |
| flatMap   | stateless | Элемент → поток элементов, без связи с другими|
| peek      | stateless | Сайд-эффект, не влияет на данные              |
| sorted    | stateful  | Требует всех элементов для сортировки         |
| limit     | stateful  | Зависит от порядка и счётчика элементов       |
| skip      | stateful  | Зависит от порядка и счётчика элементов       |
| distinct  | stateful  | Хранит увиденные элементы для сравнения       |
